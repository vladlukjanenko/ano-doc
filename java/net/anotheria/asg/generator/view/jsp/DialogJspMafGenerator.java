 ...
 * @author another
 */
public class DialogJspMafGenerator extends AbstractJSPMafGenerator {
	...
	public GeneratedJSPFile generate(MetaSection currentSection, MetaDialog dialog, MetaModuleSection section, MetaView view) {
		...
		for (int i = 0; i < elements.size(); i++) {
			...
			while (elements.get(i) instanceof MultilingualFieldElement) {
				appendString("<logic:equal name=" + quote(StrutsConfigGenerator.getDialogFormName(currentDialog, section.getDocument())) + " property="
				...
				appendString("<logic:notEqual name=" + quote(StrutsConfigGenerator.getDialogFormName(currentDialog, section.getDocument())) + " property="
						+ quote(ModuleBeanGenerator.FIELD_ML_DISABLED) + " value=" + quote("true") + ">");
				appendString("<li>");
				increaseIdent();
		...
		result += "<a href=\"#\" onClick= "
				+ quote("lightbox('All unsaved data will be lost!!!<br /> Really lock  "
						+ StrutsConfigGenerator.getDialogFormName(currentDialog, ((MetaModuleSection) currentSection).getDocument())
		...
		if (StorageType.CMS.equals(((MetaModuleSection) currentSection).getDocument().getParentModule().getStorageType())) {
			...
			appendString("<a href=\"#\" onClick= "
					...
					+ quote(StrutsConfigGenerator.getDialogFormName(currentDialog, ((MetaModuleSection) currentSection).getDocument())) + " property="
		...
		appendString("<form name=" + quote(StrutsConfigGenerator.getDialogFormName(currentDialog, ((MetaModuleSection) currentSection).getDocument()))
				+ " method=\"post\" action=" + quote(StrutsConfigGenerator.getPath(section.getDocument(), StrutsConfigGenerator.ACTION_UPDATE)) + ">");
		...
		String linksToMePagePath = StrutsConfigGenerator.getPath(section.getDocument(), StrutsConfigGenerator.ACTION_LINKS_TO_ME) + "?pId=<bean:write name="
				+ quote(StrutsConfigGenerator.getDialogFormName(currentDialog, ((MetaModuleSection) currentSection).getDocument())) + " property=\"id\"/>";
